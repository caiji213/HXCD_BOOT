...
/* SPI2 RX DMA中断服务函数 (DMA1通道0) */
void DMA1_Channel0_IRQHandler(void)
{
    if (dma_interrupt_flag_get(SPI2_DMA, SPI2_RX_DMA_CH, DMA_INT_FLAG_FTF))
    {
        dma_interrupt_flag_clear(SPI2_DMA, SPI2_RX_DMA_CH, DMA_INT_FLAG_FTF);

        // 禁用接收DMA
        dma_channel_disable(SPI2_DMA, SPI2_RX_DMA_CH);
        /* 设置接收完成标志 */
        spi_rx_complete_flag = SET;
    }
}

/* SPI2 TX DMA中断服务函数 (DMA1通道1) */
void DMA1_Channel1_IRQHandler(void)
{
    if (dma_interrupt_flag_get(SPI2_DMA, SPI2_TX_DMA_CH, DMA_INT_FLAG_FTF))
    {
        dma_interrupt_flag_clear(SPI2_DMA, SPI2_TX_DMA_CH, DMA_INT_FLAG_FTF);
        dma_channel_disable(SPI2_DMA, SPI2_TX_DMA_CH);
    }
}

// 添加中断触发标志
volatile uint32_t adc0_int_count = 0;
/* 中断服务函数在 gd32g5x3_it.c 中实现：从 ADC0 中读取三路注入结果 */
void ADC0_1_IRQHandler(void)
{
    ...
    // ADC1 处理
    if (adc_interrupt_flag_get(ADC1, ADC_INT_FLAG_EOIC))
    {
        adc_interrupt_flag_clear(ADC1, ADC_INT_FLAG_EOIC);
        iu_adc_value = adc_inserted_data_read(ADC1, 0);
    }
}

volatile uint32_t adc2_int_count = 0;
void ADC2_IRQHandler(void)
{
    adc2_int_count++;
    if (adc_interrupt_flag_get(ADC2, ADC_INT_FLAG_EOIC))
    {
        adc_interrupt_flag_clear(ADC2, ADC_INT_FLAG_EOIC);
        iv_adc_value = adc_inserted_data_read(ADC2, 0);
		udc_adc_value = adc_inserted_data_read(ADC2, 1); // 母线电压 UDC
    }
}

volatile uint32_t adc3_int_count = 0;
void ADC3_IRQHandler(void)
{
    adc3_int_count++;
    if (adc_interrupt_flag_get(ADC3, ADC_INT_FLAG_EOIC))
    {
        adc_interrupt_flag_clear(ADC3, ADC_INT_FLAG_EOIC);
        iw_adc_value = adc_inserted_data_read(ADC3, 0);
...
{
    if (timer_interrupt_flag_get(TIMER0, TIMER_INT_FLAG_CH3))
    {
...
{
    if(dma_interrupt_flag_get(VOLTAGE_ADC_DMA, VOLTAGE_ADC_DMA_CH, DMA_INT_FLAG_FTF)) {
        dma_interrupt_flag_clear(VOLTAGE_ADC_DMA, VOLTAGE_ADC_DMA_CH, DMA_INT_FLAG_FTF);

        uint16_t v_ref = voltage_adc_values[0];
        uint16_t v_type = voltage_adc_values[1];
        uint16_t v_temp = voltage_adc_values[2];
    }
}
